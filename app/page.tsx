...
export default async function Home() {
  ...
  const [goalRes, nextRes, activitiesRes, messagesRes, proposalsRes, credentialRes, pendingEvaluationsRes] = await Promise.all([
    ...
    supabase.from("coach_messages").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(60).returns<CoachMessage[]>(),
  ...
  return (
    <AppShell>
      ...
      <div className="mb-5 grid grid-cols-3 gap-2">
        <Link href="/plan" className="rounded-2xl border border-border bg-card p-3"><CalendarDays size={15} className="mb-2 text-primary" /><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Prossimo</p><p className="mt-1 truncate text-xs font-semibold">{next ? TYPE_LABELS[next.type] : "Riposo"}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{next?.date ?? "Nessuna seduta"}</p></Link>
        <Link href="/activities" className="rounded-2xl border border-border bg-card p-3"><Footprints size={15} className="mb-2 text-primary" /><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Ultimi 7 gg</p><p className="mt-1 text-xs font-semibold">{formatDistance(weeklyKm)}</p><p className="mt-0.5 text-[10px] text-muted-foreground">di corsa</p></Link>
        <Link href="/goals" className="rounded-2xl border border-border bg-card p-3"><Flag size={15} className="mb-2 text-primary" /><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Obiettivo</p><p className="mt-1 truncate text-xs font-semibold">{goal?.race_name ?? "Da impostare"}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{goal ? formatDistance(goal.distance_m) : "Parlane al coach"}</p></Link>
      </div>
