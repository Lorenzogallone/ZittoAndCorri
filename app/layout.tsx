  ...
  return (
    ...
    >
      <body className="min-h-full flex flex-col bg-background text-foreground">
        {/* Splash PWA pre-hydration: renderizzato nell'HTML del server con CSS
            inline, così appare appena arriva la risposta (prima che il JS sia
            caricato/idratato). Visibile solo in modalità standalone; lo script
            inline lo rimuove subito (prima del paint) se non serve, mentre
            PwaSplashLoader lo dissolve dopo l'hydration. */}
        ...
        <script
          // Rimuove lo splash prima del paint quando non serve: browser normale
          // (non standalone) o navigazioni successive nella stessa sessione PWA.
          dangerouslySetInnerHTML={{
            __html: `(function(){try{var s=window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===true;if(!s||sessionStorage.getItem("pwa-splash-shown")){var e=document.getElementById("pwa-splash");if(e)e.remove();}}catch(err){}})();`,
          }}
        />
        <PwaRegister />
        <PwaSplashLoader />
